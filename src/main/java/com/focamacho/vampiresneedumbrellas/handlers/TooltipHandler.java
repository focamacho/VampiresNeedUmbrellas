    ...
    public void onTooltipEvent(ItemTooltipEvent event) {
        ...
        if (item instanceof ItemUmbrella) {
            ...
            for (int i = 0; i < tooltip.size(); i++) {
                ...
                if (string.contains(Component.translatable("item.modifiers.mainhand").getString())) {
                    indexToReplace = i;
                } else if (string.contains(Component.translatable("item.modifiers.offhand").getString())) {
            ...
            if(indexToReplace != -1) event.getToolTip().set(indexToReplace, Component.translatable("modifiers.vampiresneedumbrellas.use").withStyle(ChatFormatting.GRAY));
